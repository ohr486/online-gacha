class Admin::TweetTemplatesController < Admin::ApplicationController
  before_action :set_tweet_template, only: [:show, :edit, :update, :destroy]

  # GET /tweet_templates
  # GET /tweet_templates.json
  def index
    @tweet_templates = TweetTemplate.all.order(tweet_day: "ASC")
  end
  ...
  def create
    ...
    respond_to do |format|
      if @tweet_template.save
        format.html { redirect_to [:admin, @tweet_template], notice: 'Tweet template was successfully created.' }
        format.json { render :show, status: :created, location: [:admin, @tweet_template] }
      else
  ...
  def update
    respond_to do |format|
      if @tweet_template.update(tweet_template_params)
        format.html { redirect_to [:admin, @tweet_template], notice: 'Tweet template was successfully updated.' }
        format.json { render :show, status: :ok, location: [:admin, @tweet_template] }
      else
  ...
  private
    ...
    def tweet_template_params
      params.require(:tweet_template).permit(:tweet_day, :message, :link)
    end
end
